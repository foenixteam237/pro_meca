...
class _ClientVehicleFormPageState extends State<ClientVehicleFormPage> {
  ...
  Future<void> _selectImageSource({bool isVehicleImage = true}) async {
    ...
    if (action != null) {
      try {
        ...
        } else if (action == ImageSource.gallery &&
              ...
              if (isVehicleImage) {
                ...
                if (_vehicleConditionImages.length < 4) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Maximum 4 images autorisées'),
                    ),
                  );
                }
              }
  ...
  void _submitForm() async {
    if (!_isFormValid()) {
      ...
      if (_selectedImage == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Veuillez ajouter une image du véhicule'),
          ),
        );
      }
      return;
    ...
    try {
      ...
      if (clientId.isNotEmpty) {
        ...
        FormData vehicleFormData = FormData.fromMap(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Réception du véhicule"), elevation: 2),
      body: Padding(
                          ...
                          children: [
                            ...
                            Positioned(
                              ...
                              child: IconButton(
                                ...
                                onPressed: () =>
                                    _removeConditionImage(entry.key),
                              ),
                            ),
                          ],
                        );
  ...
  void _viewImage(File image) {
    showDialog(
      context: context,
      builder: (context) => Dialog(child: PhotoView(image: image)),
    );
  }
        ...
        Expanded(
          child: ElevatedButton(
            ...
            child: isLoading
                ...
                : Text("Terminé", style: AppStyles.buttonText(context)),
          ),
        ),
      ],
    );
